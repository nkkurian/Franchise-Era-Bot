...
// --- 3. DEFINE SLASH COMMANDS (4-BOX TRADE INCLUDED) ---
const commands = [
...
client.on('interactionCreate', async (interaction) => {
  ...
  // CRITICAL: Defer immediately to prevent "Application did not respond" (3-second timeout fix)
  ...
  try {
      ...
      if (playerRow) {
        ...
        const salaryEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: '📝 Extended', value: playerRow.get('Extended') === 'TRUE' ? "✅ Yes" : "❌ No", inline: true },
    ...
    if (commandName === 'team') {
      ...
      if (sheet) {
        ...
        const teamEmbed = new EmbedBuilder()
          ...
          .setColor(0x3498db)
          .addFields(
            { name: '💸 Cap Space', value: capSpace, inline: true },
            { name: '📝 Extensions Left', value: extensions, inline: true }
          );
    ...
    if (commandName === 'trade') {
      ...
      const processTradeSide = async (name, playerNames) => {
        ...
        playerNames.forEach(pName => {
          ...
          if (row) {
            ...
            totalHit += hit;
            details.push(`- **${actualName}** ($${hit.toLocaleString()})${dead ? " 💀" : ""}`);
          }
        });
      ...
      if (sideA.details.some(d => d.includes("💀")) || sideB.details.some(d => d.includes("💀"))) {
        tradeEmbed.setFooter({ text: '⚠️ Warning: One or more players carry Dead Cap penalties.' });
      }
    ...
    await interaction.editReply("⚠️ There was an error processing your request. Check your spreadsheet data and formatting.");
  }
});
